 ...
 * BetInitialize Slice
 *
 * Manages betInitialize RPC request/response data with persistence across navigation
 */

import { createSlice } from '@reduxjs/toolkit';
import type { PayloadAction } from '@reduxjs/toolkit';

export interface BetInitializeState {
  request: {
    method: string;
    params: unknown;
  } | null;
...
const initialState: BetInitializeState = {
  request: null,
  response: null,
  rawResponse: null,
  error: null,
...
const betInitializeSlice = createSlice({
  name: 'betInitialize',
  initialState,
  reducers: {
    ...
    setBetInitializeError: (state, action: PayloadAction<{ error: string; duration: number }>) => {
      state.error = action.payload.error;
      state.duration = action.payload.duration;
      state.response = null;
      state.rawResponse = null;
    },
    clearBetInitializeData: (state) => {
      state.request = null;
      state.response = null;
      state.rawResponse = null;
      state.error = null;
