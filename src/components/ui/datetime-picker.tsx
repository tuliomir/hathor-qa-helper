 ...
 * Transformed to use DaisyUI styling patterns
 */
...
// Custom Calendar Component with month/year selectors
function Calendar({
  className,
  classNames,
    ...
    <div className="p-3">
      ...
      <div className="flex justify-center gap-2 mb-3">
        <select
          ...
          value={props.month?.getMonth() ?? new Date().getMonth()}
          onChange={(e) => {
            const newDate = new Date(props.month ?? new Date());
            newDate.setMonth(Number.parseInt(e.target.value, 10));
        ...
        <select
          className="input text-xs py-1 px-2 w-auto"
          value={props.month?.getFullYear() ?? new Date().getFullYear()}
          onChange={(e) => {
            const newDate = new Date(props.month ?? new Date());
            newDate.setFullYear(Number.parseInt(e.target.value, 10));
            props.onMonthChange?.(newDate);
          }}
        >
          {YEARS.map((year) => (
      ...
      <DayPicker
        ...
        classNames={{
          ...
          month: 'space-y-4',
          month_caption: 'hidden', // We use custom selects above
          month_grid: 'w-full border-collapse',
          weekdays: 'flex',
          weekday: 'text-muted text-xs font-medium w-9 text-center',
          week: 'flex w-full mt-1',
          day: 'h-9 w-9 p-0 font-normal text-center text-sm hover:bg-primary hover:text-white rounded cursor-pointer',
          day_button: 'h-9 w-9',
          selected: 'bg-primary text-white hover:bg-primary-dark',
          today: 'bg-warning text-black font-bold',
...
const TimePeriodSelect = React.forwardRef<HTMLSelectElement, PeriodSelectorProps>(
    ...
    return (
      <div className="flex h-10 items-center">
        <select
          ...
          value={period}
          onChange={(e) => handleValueChange(e.target.value as Period)}
  ...
  (
    ...
    return (
      <input
        ...
        id={id || picker}
        name={name || picker}
        className={`input w-12 text-center font-mono text-base tabular-nums caret-transparent ${className || ''}`}
  ...
  (
    ...
    return (
      <div className="relative w-full">
        <button
          type="button"
          className={`input flex items-center justify-start text-left font-normal ${!displayDate ? 'text-gray-500' : ''} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} ${className || ''}`}
          ref={buttonRef}
        ...
        {isOpen && (
          <div
            ref={dropdownRef}
            className="absolute z-50 mt-2 bg-white border-2 border-primary rounded-lg shadow-lg"
