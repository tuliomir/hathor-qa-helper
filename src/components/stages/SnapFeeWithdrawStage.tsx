...
export const SnapFeeWithdrawStage: React.FC = () => {
  ...
  useEffect(() => { if (snapAddress && !withdrawAddress) setWithdrawAddress(snapAddress); }, [snapAddress]);

  const params = useMemo(() => {
    const actions: Array<Record<string, unknown>> = [];
    if (contractPaysFees && htrWithdrawAmount) {
      actions.push({ type: 'withdrawal', token: '00', amount: htrWithdrawAmount, address: withdrawAddress, changeAddress: withdrawAddress });
    }
    actions.push({ type: 'withdrawal', token: feeToken, amount, address: withdrawAddress, changeAddress: withdrawAddress });
    return {
      method: 'noop',
      nc_id: ncId,
      blueprint_id: null,
  ...
  return (
    <div className="max-w-300 mx-auto">
      ...
      <p className="text-muted mb-7.5">
        Withdraw a fee-based token from a Fee nano contract via MetaMask Snap
      </p>
      ...
      {isSnapConnected && (
        ...
        >
          ...
          <div>
            ...
            <input type="text" value={feeToken} onChange={(e) => setFeeToken(e.target.value)} placeholder="Token UID" className="input" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1.5">Amount</label>
            <input type="text" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Amount to withdraw" className="input" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1.5">Withdrawal Address</label>
            <input type="text" value={withdrawAddress} onChange={(e) => setWithdrawAddress(e.target.value)} placeholder="Your address" className="input" />
          </div>
          ...
          {contractPaysFees && (
            <div>
              ...
              <input type="text" value={htrWithdrawAmount} onChange={(e) => setHtrWithdrawAmount(e.target.value)} placeholder="HTR amount to withdraw for fees" className="input" />
            </div>
          )}
        </SnapMethodCard>
      )}
