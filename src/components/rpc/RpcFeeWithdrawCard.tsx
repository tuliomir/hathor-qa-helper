/**
 * RPC Fee Withdraw Card Component
 *
 * Card for testing withdrawing fee-based tokens from a fee nano contract via RPC call
 */
...
export interface RpcFeeWithdrawCardProps {
...
export const RpcFeeWithdrawCard: React.FC<RpcFeeWithdrawCardProps> = ({
  onExecute,
  disabled = false,
  isDryRun = false,
  ncId,
  ...
  useEffect(() => {
    ...
    const actions: { type: string; token: string; amount: string; address: string }[] = [];

    if (contractPaysFees) {
      actions.push({
        type: 'withdrawal',
        token: '00',
        amount: htrAmount.toString(),
  ...
  }, [ncId, withdrawAddress, amount, feeToken, pushTx, contractPaysFees, htrWithdrawAmount]);

  const handleSelectLatestNcId = () => {
    if (!latestNcId) {
      showToast('No nano contract has been initialized yet. Please initialize a fee contract first.', 'error');
      return;
    }
    setNcId(latestNcId);
      ...
      <div className="card-primary mb-7.5">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted mt-1">
              The nano contract ID to withdraw from. Click "Select" to use the latest initialized contract.
            </p>
          </div>
          ...
          {/* HTR Withdraw Amount (only when contract pays fees) */}
          {contractPaysFees && (
            <div>
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          <button onClick={handleExecute} disabled={loading || disabled} className="btn-primary">
            {loading ? 'Withdrawing...' : 'Withdraw Fee Token'}
          </button>
        </div>
      </div>
        ...
        <div className="card-primary mb-7.5">
          <div className="bg-green-50 border border-green-300 rounded p-4">
            <div className="flex items-center gap-2 text-green-700 mb-3">
              ...
              <span className="font-medium">Fee token withdrawn successfully</span>
            </div>
            <div>
