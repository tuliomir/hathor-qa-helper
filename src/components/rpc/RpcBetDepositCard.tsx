...
export const RpcBetDepositCard: React.FC<RpcBetDepositCardProps> = ({
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      showToast(isDryRun ? 'Request generated (not sent to RPC)' : 'Bet placed successfully', 'success');
    } catch (err: unknown) {
  ...
  const renderRawJson = (data: unknown) => {
    return (
      <div className="border border-gray-300 rounded p-3 overflow-auto max-h-96 bg-gray-50 text-left">
        <pre className="text-sm font-mono whitespace-pre-wrap break-words m-0 text-left">{safeStringify(data, 2)}</pre>
      </div>
    );
  };
      ...
      <div className="card-primary mb-7.5 relative">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted mt-1">Place a bet on an existing bet nano contract</p>
          </div>
          {isDryRun && (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="input"
            />
            <p className="text-xs text-muted mt-1">Your prediction/choice for the bet outcome</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="input"
            />
            <p className="text-xs text-muted mt-1">Amount to bet (in token's smallest unit)</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="input"
            />
            <p className="text-xs text-muted mt-1">Index of the address to use for placing the bet</p>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-muted mt-1">Token used for placing the bet</p>
            {tokenMismatch && (
              <div className="mt-2 text-xs text-warning flex items-start gap-1">
                ...
                <span>
                  Warning: This token ({token}) is different from the one used to initialize the nano contract (
                  {initialToken})
                </span>
              </div>
            )}
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white border border-yellow-200 rounded overflow-hidden">
                ...
                <div className="px-3 py-2">
                  {address ? (
                    <span className="text-sm font-mono text-yellow-900 break-all">{address}</span>
                  ) : (
                    <span className="text-sm text-muted italic">Deriving address from wallet...</span>
                  )}
                </div>
              </div>
            </div>
